    ...
    equipmentData = [
        {
            ...
            category: "ふく",
            stars: 5,
            image: "https://d.kuku.lu/wkbkkgfsu",
            effect: "クリティカル+100%",
            tags: ["クリティカル"]
        },
    ];
...
// 新機能: タグ検索と☆の数検索のロジック追加
function applyAdvancedSearch() {
    const tagSearchBox = document.getElementById('tagSearchBox').value.trim().toLowerCase();
    const starFilter = document.getElementById('starFilter').value;

    // タグを分割してAND条件にする
    const tags = tagSearchBox.split(',').map(tag => tag.trim()).filter(tag => tag);

    const filteredEquipment = equipmentData.filter(item => {
        const tagMatch = tags.every(tag => item.tags.map(t => t.toLowerCase()).includes(tag));
        const starMatch = starFilter ? item.stars === parseInt(starFilter) : true;
